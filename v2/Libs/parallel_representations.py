...
from Repres_utils import bonds_to_graph,find_all_paths,build_i_idx,ordered_charges,distmat
...
def add_repr2mols(calcs,coordinate_type):
    ...
    for calc in calcs:
        mol=[]
        charges,xyzcoords,bonds,idxs,q,g_ic,h_ic=calc
        ...
        for b in range(len(q)):  # looping over the coordinates
            ...
            if len(idx)==2 and coordinate_type=="bonds": #bond
             ...
             #   if not ordered_charges([charges[x] for x in molg[i]],[charges[x] for x in molg[j]] ):i,j=j,i
                ...
                repres=build_bond_repr(charges,xyzcoords,(i,j),molg,DM)
                repres.append(h_ic[b,b])

                mol.append([label,repres])
                ...
                repres=build_angle_repr(charges,xyzcoords,[i,j,k],i_idxs,q,b,molg)
                repres.append(h_ic[b,b])
