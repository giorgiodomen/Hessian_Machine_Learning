...
def atom_env(i,j,molgi,charges,DM,xyzcoords):
    env_i=[0]*(3*3)
    envi_list=[]
    for atom in molgi:
        envi_list.append([charges[atom],DM[atom,i], angle_cos(xyzcoords,(j,i,atom))] )
    envi_list.sort(reverse=True)
    envi_list=np.asarray(envi_list).flatten()
    env_i[:min(9,len(envi_list))] =envi_list[:min(9,len(envi_list))]
    return env_i

def build_bond_repr(charges,xyzcoords,idx,molg,DM):
    ...
    env_i=atom_env(i,j,molgi,charges,DM,xyzcoords)
    bond_repr+=env_i
    
    for k in molgi:
        molgk=molg[k].copy()
        molgk.remove(i)
        env_k=atom_env(k,i,molgk,charges,DM,xyzcoords)
        bond_repr+=env_k
    bond_repr+=[0]*9*(4-len(molgi))

    #env J)
    molgj=molg[j].copy()
    if i in molgj: molgj.remove(i)
    env_j=atom_env(j,i,molgj,charges,DM,xyzcoords)
    bond_repr+=env_j
    ...
    for k in molgj:
        molgk=molg[k].copy()
        molgk.remove(j)
        env_k=atom_env(k,j,molgk,charges,DM,xyzcoords)
        bond_repr+=env_k
    bond_repr+=[0]*9*(4-len(molgj))
